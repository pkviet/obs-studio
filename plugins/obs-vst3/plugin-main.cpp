...
bool obs_module_load(void)
{
	...
	if (obs_get_nix_platform() != OBS_NIX_PLATFORM_X11_EGL) {
		blog(LOG_WARNING, "[obs-vst3] X11 note detected; disabling VST3 host");
		return false;
	}
#endif
	if (!load_host()) {
	...
	if (!retrieve_vst3_list()) {
		blog(LOG_INFO, "OBS-VST3: you'll have to install VST3s in order to use this filter.");
	}
