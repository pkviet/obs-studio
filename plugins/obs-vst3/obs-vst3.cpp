...
bool load_host()
{
	...
	if (!g_display) {
		blog(LOG_WARNING, "[obs-vst3] no X11 display; disabling VST3 host");
		return false;
	}
#endif
...
#ifdef __linux__
	g_run_loop = new RunLoopImpl(static_cast<Display *>(g_display));
		...
		if (needs_resampling && sc_resampler) {
			...
			if (audio_resampler_resample(sc_resampler.get(), resampled, &out_frames, &ts_offset,
				...
				for (int ch = 0; ch < sc_num_channels; ++ch) {
					...
					float *vstIn = plugin->auxChannelBuffer32(Steinberg::Vst::kInput, ch);
					if (inBuf && vstIn) {
...
static obs_properties_t *vst3_properties(void *data)
{
	...
	obs_property_t *gui =
		obs_properties_add_button2(props, S_EDITOR, obs_module_text(TEXT_EDITOR), vst3_show_gui_callback, NULL);
	obs_property_set_visible(gui, !vd->noview.load(std::memory_order_relaxed) && !vd->last_init_failed);
